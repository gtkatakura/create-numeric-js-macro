...
function createNumericMacro({
  ...
  function macro({ references, state, babel }) {
    ...
    const libraryIdentifier = program.scope.generateUidIdentifier(identifierName)

    references.default.forEach((referencePath) => {
      if (referencePath.parentPath.type === 'CallExpression') {
        asFunction(referencePath.parentPath.get('arguments'), state, babel, libraryIdentifier)
      } else {
        throw new Error(libraryName + ' can only be called as a function')
      }
    })
  ...
  function importLibrary({ references, state, babel }, libraryIdentifier) {
    ...
    program.node.body.unshift(t.importDeclaration(
      [t.importSpecifier(libraryIdentifier, t.identifier(identifierName))],
      t.stringLiteral(libraryTarget),
    ))
  }
